...
describe('toFailWith', () => {
    test('returns true for a failure result and matching string, RegExp or undefined and a matching detail', () => {
        ...
        expect(predicate(failWithDetail(undefined as unknown as string, 'detail'), undefined, 'detail')).toBe(true);
    });
    ...
    test('returns false for a failure result and matching string, RegExp or undefined but with a mismatched detail', () => {
        ...
        expect(predicate(failWithDetail('oops', { detail: 'other detail' }), /o.*/i, { detail: 'detail' })).toBe(false);
        expect(predicate(failWithDetail(undefined as unknown as string, 'other detail'), undefined, 'detail')).toBe(false);
    });
});
