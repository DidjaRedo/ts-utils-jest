...
describe('toFailExpectations', () => {
    test('returns success with true for a callback that fails', () => {
        expect(predicate(() => {
            expect(true).toBe(false);
        })).toSucceedWith(expect.stringMatching(
            /expect/i
        ));
    });
    ...
    test('returns success with the returned string for a callback that fails with an expected string', () => {
        ...
        }, 'This is an error')).toSucceedWith('This is an error');
    });
